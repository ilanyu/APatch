...
import me.hidden.hidden.ui.theme.refreshTheme
import me.hidden.hidden.util.APatchKeyHelper
import me.hidden.hidden.util.getBugreportFile
import me.hidden.hidden.util.hideapk.HideAPK
import me.hidden.hidden.util.isGlobalNamespaceEnabled
import me.hidden.hidden.util.outputStream
import me.hidden.hidden.util.rootShellForResult
import me.hidden.hidden.util.setGlobalNamespaceEnabled
import me.hidden.hidden.util.ui.APDialogBlurBehindUtils
import me.hidden.hidden.util.ui.LocalSnackbarHost
import me.hidden.hidden.util.ui.NavigationBarsSpacer
...
fun SettingScreen() {
    val state by MyApp.apStateLiveData.observeAsState(MyApp.State.UNKNOWN_STATE)
    val kPatchReady = state != MyApp.State.UNKNOWN_STATE
    val aPatchReady =
        (state == MyApp.State.ANDROIDPATCH_INSTALLING || state == MyApp.State.ANDROIDPATCH_INSTALLED || state == MyApp.State.ANDROIDPATCH_NEED_UPDATE)
    ...
    Scaffold(
        ...
        Column(
            ...
            val context = LocalContext.current
            val scope = rememberCoroutineScope()
            val prefs = MyApp.sharedPreferences
            ...
            if (aPatchReady) {
                ...
                SwitchItem(
                    ...
                    MyApp.sharedPreferences.edit().putBoolean("enable_web_debugging", it)
                        .apply()
            ...
            Column(modifier = Modifier.padding(PaddingValues(all = 24.dp))) {
                ...
                Row(
                    ...
                    Button(enabled = suPathChecked(suPath), onClick = {
                        ...
                        rootShellForResult("echo $suPath > ${MyApp.SU_PATH_FILE}")
                    }) {
