...
import me.hidden.hidden.MyApp
import me.hidden.hidden.Natives
import me.hidden.hidden.R
import me.hidden.hidden.apApp
import me.hidden.hidden.ui.component.ProvideMenuShape
import me.hidden.hidden.ui.component.rememberConfirmDialog
import me.hidden.hidden.ui.viewmodel.PatchesViewModel
import me.hidden.hidden.util.LatestVersionInfo
import me.hidden.hidden.util.Version
import me.hidden.hidden.util.Version.getManagerVersion
import me.hidden.hidden.util.checkNewVersion
import me.hidden.hidden.util.getSELinuxStatus
import me.hidden.hidden.util.reboot
import me.hidden.hidden.util.ui.APDialogBlurBehindUtils
...
fun HomeScreen(navigator: DestinationsNavigator) {
    ...
    val kpState by MyApp.kpStateLiveData.observeAsState(MyApp.State.UNKNOWN_STATE)
    val apState by MyApp.apStateLiveData.observeAsState(MyApp.State.UNKNOWN_STATE)

    if (kpState != MyApp.State.UNKNOWN_STATE) {
    ...
    Scaffold(topBar = {
        ...
        Column(
            ...
            if (kpState != MyApp.State.UNKNOWN_STATE && apState != MyApp.State.ANDROIDPATCH_INSTALLED) {
                AStatusCard(apState)
            }
            val checkUpdate = MyApp.sharedPreferences.getBoolean("check_update", true)
            if (checkUpdate) {
                UpdateCard()
            }
...
@Composable
private fun TopBar(
    onInstallClick: () -> Unit, navigator: DestinationsNavigator, kpState: MyApp.State
) {
...
private fun KStatusCard(
    kpState: MyApp.State, apState: MyApp.State, navigator: DestinationsNavigator
) {
    ...
    val cardBackgroundColor = when (kpState) {
        MyApp.State.KERNELPATCH_INSTALLED -> {
            MaterialTheme.colorScheme.primary
        }

        MyApp.State.KERNELPATCH_NEED_UPDATE, MyApp.State.KERNELPATCH_NEED_REBOOT -> {
    ...
    ElevatedCard(
        onClick = {
            if (kpState != MyApp.State.KERNELPATCH_INSTALLED) {
                navigator.navigate(InstallModeSelectScreenDestination)
            }
        },
        colors = CardDefaults.elevatedCardColors(containerColor = cardBackgroundColor),
        elevation = CardDefaults.cardElevation(
            defaultElevation = if (kpState == MyApp.State.UNKNOWN_STATE) 0.dp else 6.dp
        )
    ) {
        Column(
            modifier = Modifier
            ...
            if (kpState == MyApp.State.KERNELPATCH_NEED_UPDATE) {
                Row {
                    Text(
            ...
            Row(
                ...
                Column(
                    ...
                    if (kpState != MyApp.State.UNKNOWN_STATE && kpState != MyApp.State.KERNELPATCH_NEED_UPDATE && kpState != MyApp.State.KERNELPATCH_NEED_REBOOT) {
                        Spacer(Modifier.height(4.dp))
                        Text(
                            text = "${Version.installedKPVString()} (${managerVersion.second}) - " + if (apState != MyApp.State.ANDROIDPATCH_NOT_INSTALLED) "Full" else "KernelPatch",
                            style = MaterialTheme.typography.bodyMedium
                        )
                    }
                }
                ...
                Column(
                    ...
                    Button(onClick = {
                        when (kpState) {
                            MyApp.State.UNKNOWN_STATE -> {
                                showAuthKeyDialog.value = true
                            }

                            MyApp.State.KERNELPATCH_NEED_UPDATE -> {
                            ...
                            MyApp.State.KERNELPATCH_NEED_REBOOT -> {
                                reboot()
                            }

                            MyApp.State.KERNELPATCH_UNINSTALLING -> {
                                // Do nothing
                            }

                            else -> {
                                if (apState == MyApp.State.ANDROIDPATCH_INSTALLED || apState == MyApp.State.ANDROIDPATCH_NEED_UPDATE) {
...
@Composable
private fun AStatusCard(apState: MyApp.State) {
    ElevatedCard(
        ...
        Column(
            ...
            Row(
                ...
                if (apState != MyApp.State.UNKNOWN_STATE) {
                    Column(
                        modifier = Modifier.align(Alignment.CenterVertically)
                    ) {
                        Button(onClick = {
                            when (apState) {
                                MyApp.State.ANDROIDPATCH_NOT_INSTALLED, MyApp.State.ANDROIDPATCH_NEED_UPDATE -> {
                                    MyApp.installApatch()
                                }

                                MyApp.State.ANDROIDPATCH_UNINSTALLING -> {
                                    // Do nothing
                                }
...
@Composable
private fun InfoCard(kpState: MyApp.State, apState: MyApp.State) {
    ElevatedCard {
        Column(
            modifier = Modifier
                .fillMaxWidth()
            ...
            if (apState != MyApp.State.UNKNOWN_STATE && apState != MyApp.State.ANDROIDPATCH_NOT_INSTALLED) {
                InfoCardItem(
