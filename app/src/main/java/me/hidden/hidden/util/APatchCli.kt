...
import me.hidden.hidden.MyApp.Companion.SUPERCMD
import me.hidden.hidden.BuildConfig
import me.hidden.hidden.apApp
import me.hidden.hidden.ui.screen.MODULE_TYPE
import java.io.File
...
fun createRootShell(): Shell {
    ...
    return try {
        builder.build(
            SUPERCMD, MyApp.superKey, "-Z", MyApp.MAGISK_SCONTEXT
        )
    } catch (e: Throwable) {
        ...
        return try {
            ...
            builder.build(
                getKPatchPath(), MyApp.superKey, "su", "-Z", MyApp.MAGISK_SCONTEXT
            )
        } catch (e: Throwable) {
...
fun execApd(args: String): Boolean {
    val shell = getRootShell()
    return ShellUtils.fastCmdResult(shell, "${MyApp.APD_PATH} $args")
}

fun listModules(): String {
    val shell = getRootShell()
    val out =
        shell.newJob().add("${MyApp.APD_PATH} module list").to(ArrayList(), null).exec().out
...
fun installModule(
    ...
    with(resolver.openInputStream(uri)) {
        ...
        if(type == MODULE_TYPE.APM) {
            val cmd = "${MyApp.APD_PATH} module install ${file.absolutePath}"
...
fun runAPModuleAction(
    ...
    val result = shell.newJob().add("${MyApp.APD_PATH} module action $moduleId")
...
fun isGlobalNamespaceEnabled(): Boolean {
    val shell = getRootShell()
    val result = ShellUtils.fastCmd(shell, "cat ${MyApp.GLOBAL_NAMESPACE_FILE}")
    Log.i(TAG, "is global namespace enabled: $result")
    return result == "1"
}

fun setGlobalNamespaceEnabled(value: String) {
    getRootShell().newJob().add("echo $value > ${MyApp.GLOBAL_NAMESPACE_FILE}")
        .submit { result ->
